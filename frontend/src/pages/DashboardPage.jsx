...
export default function DashboardPage() {
  ...
  return (
    <div 
      className="min-h-screen bg-[#F8FAFC] flex text-slate-900 overflow-hidden" 
      style={{ 
    ...
    >
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Inter:opsz,wght@14..32,400;500;600;700;800;900&display=swap');`}</style>
      ...
      <main className="flex-1 flex flex-col relative overflow-y-auto w-full">
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200/50 px-4 sm:px-6 lg:px-8 flex items-center justify-between sticky top-0 z-10">
          <div className="relative w-full max-w-xs sm:max-w-sm md:max-w-md group">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-indigo-600 transition-colors" size={18} />
            <input 
              type="text" 
              placeholder="Rechercher..." 
              className="w-full pl-12 pr-4 py-2.5 bg-slate-100/50 border-none rounded-2xl focus:bg-white focus:ring-4 focus:ring-indigo-500/5 outline-none transition-all font-medium text-sm" 
            />
          </div>
        </header>
        ...
        <div className="p-4 sm:p-6 lg:p-8 max-w-7xl w-full mx-auto">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-10">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
              <h2 className="text-3xl sm:text-4xl font-[800] text-slate-900 tracking-[-0.02em] leading-[1.1]">
                Bonjour <span className="text-indigo-600">!</span>
              </h2>
              <p className="text-slate-500 font-medium text-base sm:text-lg leading-[1.6]">Prêt à décrocher votre prochain job ?</p>
            </motion.div>
            ...
            <motion.button 
              ...
              whileTap={{ scale: 0.98 }}
              className="w-full sm:w-auto bg-indigo-600 text-white px-6 sm:px-8 py-3 sm:py-4 rounded-2xl font-bold tracking-[-0.01em] flex items-center justify-center gap-3 hover:bg-indigo-700 shadow-xl shadow-indigo-600/25"
            >
          ...
          {/* TABLE */}
          <div className="bg-white border border-slate-200/60 rounded-[24px] sm:rounded-[32px] overflow-hidden shadow-sm">
            <div className="p-4 sm:p-6 lg:p-8 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-slate-50/30">
              <h3 className="font-bold text-lg sm:text-xl tracking-[-0.01em]">Documents récents</h3>
              <button onClick={() => navigate('/cvs')} className="text-indigo-600 text-sm font-bold tracking-[-0.01em] flex items-center gap-1 hover:gap-2 transition-all">
                ...
                {loading ? (
                  ...
                  documents.map((doc) => (
                    <motion.div 
                      ...
                      className="p-4 sm:p-6 hover:bg-slate-50 transition-all flex items-center justify-between gap-4 group cursor-pointer"
                    >
                      <div className="flex items-center gap-4 sm:gap-5">
                        <div className="w-10 h-10 sm:w-12 sm:h-12 bg-white shadow-sm border border-slate-100 rounded-xl flex items-center justify-center text-indigo-600">
                          {doc.type === 'CV' ? <FileText size={20} /> : <Mail size={20} />}
                        </div>
                        <div>
                          <h4 className="font-bold text-slate-900 tracking-[-0.01em] group-hover:text-indigo-600 text-sm sm:text-base">{doc.title}</h4>
                  ...
                  <div className="p-10 sm:p-16 text-center">
                    <div className="w-16 h-16 sm:w-20 sm:h-20 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-4 border border-slate-100">
                      <FileText className="text-slate-200" size={28} />
                    </div>
                    <p className="text-slate-500 font-semibold text-base sm:text-lg">Aucun document pour le moment</p>
                    <p className="text-slate-400 text-sm mb-6">Commencez par créer votre premier CV professionnel.</p>
                    <button 
                      onClick={() => navigate('/cvs/templates')}
                      className="text-indigo-600 font-bold tracking-[-0.01em] bg-indigo-50 px-6 py-2.5 rounded-xl hover:bg-indigo-100 transition-colors"
                    >
                      Créer maintenant
                    </button>
                  </div>
...
function StatCard({ icon, title, desc, value, onClick }) {
  return (
    <motion.div onClick={onClick} whileHover={{ y: -5 }} className="p-6 sm:p-8 bg-white border border-slate-200/60 rounded-[24px] sm:rounded-[32px] shadow-sm hover:shadow-xl transition-all cursor-pointer group">
      <div className="w-10 h-10 sm:w-12 sm:h-12 bg-slate-50 rounded-xl flex items-center justify-center mb-4 sm:mb-6 group-hover:bg-indigo-600 group-hover:text-white transition-colors">
        {React.cloneElement(icon, { size: 22, className: "group-hover:text-white" })}
      </div>
      <div className="flex items-center justify-between mb-1">
        <h4 className="font-[800] text-lg sm:text-xl text-slate-900 tracking-[-0.01em]">{title}</h4>
        {value && <span className="text-lg sm:text-xl font-black text-indigo-600">{value}</span>}
      </div>
      <p className="text-sm text-slate-500 font-medium leading-[1.5]">{desc}</p>
    </motion.div>
  );
}